...
*/
var tmp;
/* Copied from peerWidget */
(function($) {
$.widget("cow.P2PWidget", {
	options: {
 ...
 _create: function() {
		...
		element.delegate('.owner','click', function(){
			...
			conn = self.pjs.connect("pjs" + owner);
			conn.on('open', function(){
				console.log("Peerjs connection to " + owner + " established");
				conn.send('hi!');
				var peerConnection = self.pjs.managers["pjs"+owner].pc;
				call(peerConnection);
			});
			conn.on('close', function(){
				console.log("Peerjs connection to " + owner + " closed");
			});
	...
	_onConnect: function(evt) {
		...
		self.pjs = new Peer("pjs" + core.me().uid, {key: '3ryspwpldblc8fr'});
		self.pjs.on('connection', function(conn) {
	...
	_updateList: function(evt) {		
		...
		var names = 'Local:<video id="localVideo" autoplay></video><br>Remote:<video id="remoteVideo" autoplay></video>';
		...
		$.each(peers,function(){
			...
			names = names+ '<span class="peerlist owner" title="click to see this peers view" owner="'+this.uid+'">'+this.options.owner+'<video id="webcampjs'+this.options.owner+'"></video></span></br>';
			}
		});
		element.html(
			names
